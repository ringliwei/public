...
var handler = {
    ...
    ajaxHook({
      open: function ([_, url], xhr) {
        if (url.indexOf("playurl?") > 0) {
          ...
          storage.playinfoUrl = url.startsWith("http")
            ? url
  ...
  "qq.com": function () {
    ajaxHook({
      ...
      send: ([body], xhr) => {
        if (
          xhr.url.includes("qq.com/proxyhttp") &&
          body.includes("vinfoparam")
        ) {
          xhr.body = body;
        }
      },
...
async function bilibili_parseResult(rs) {
  ...
  if (storage.playinfoUrl) {
    ...
    var playinfoBaseUrl = storage.playinfoUrl.replace(
      /&cid=[^&]+?$|cid=.+?&/,
      ""
    );
    var sessCookie = document.cookie
      .split("; ")
    ...
    var tips = sessCookie
      ? ""
      : "未登录或cookie中的SESSDATA项的HttpOnly属性为true，不一定支持最高清晰度";
...
async function bilibili_singlePart(data) {
  ...
  if (data.dash) {
    ...
    data.dash.video.sort(sortBw).forEach((video) => {
      ...
      var html = `${width}x${height}  ${fileformat}  ${timelength}分钟  ${buildLink(
        url
      )}`;
      htmls.push(html);
    });
    ...
    htmls = playinfoList.map((playinfo) => {
      ...
      return `${fileformat}  ${timelength}分钟  ${size}MB  分${
        urls.length
      }段  ${buildLink(url)}`;
    });
  }
...
function mgtv_parseResult(rs) {
  ...
  Promise.all(tasks).then((rsList) => {
    ...
    rsList.forEach((rs) => {
      ...
      html += `${width}x${height}  ${fileformat}  ${name}  ${size}M  ${buildLink(
        url
      )}\n`;
    });
...
function prepare() {
  ...
  Object.assign($, {
    ...
    logEmphasize: function () {
      ...
      args.splice(
        ...
        1,
        "%c" + args[0],
        "color:green;font-size:1.3em;font-weight:bold;background:#abfdc1;"
      );
      console.log(...args);
    },
  });
    ...
    on: function (event, arg1, arg2) {
      if (event === "childListChanged") {
        ...
        var [selector, listener] =
          arg2 instanceof Function ? [arg1, arg2] : [null, arg1];
  ...
  $.ready(() => {
    ...
    $.addStyle(
      "https://cdn.bootcdn.net/ajax/libs/font-awesome/4.0.0/css/font-awesome.min.css"
    );
    $.addStyle(`
        .swal2-container {
          font-size: 18px;
